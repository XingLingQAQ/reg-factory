...
## 2026-08-31 - 2.1.0（独立 K12 新适配器）

**Codex K12 新实现**
- 新建独立 `k12/` Python/FastAPI 服务与原生前端，不复用旧 `codex_k12` 应用代码或数据模型。
- K12 保持独立端口 `8806`、独立任务队列和 `runtime/k12` 数据目录；主 WebUI 仅负责检测、启动、停止和入口。
- 复用项目已有邮箱解析、浏览器、OAuth、Sub2API、代理和任务执行基础能力，支持邮箱导入、Workspace request/accept、批量任务、取消和日志查看。
- 便携包支持 `start_k12.bat` / `start_k12.sh` 单独启动 K12。
