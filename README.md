...
Codex K12 已完全融合在主 WebUI 与便携包中，不需要单独安装、启动 Node 服务或使用额外端口。
...
- Node.js 20+（仅开发源码或 Gmail Android 模块按需需要）
...
- Codex / K12：在统一的 Plus/Codex 工作台管理 workspace、邮箱资产、OAuth、Sub2API 和 Codex 凭据。
...
控制台只监听本机，K12 与其它功能共用同一端口、任务队列和运行数据目录。
